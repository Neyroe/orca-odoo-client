...
describe('capturing the instance behind a rejected API key', () => {
  it('names the instance from a failed list read before the status wipe', async () => {
    odooListTickets.mockRejectedValue(new Error('AccessDenied: invalid API key'))
  ...
  it('leaves a record-permission failure alone', async () => {
    ...
    odooListTickets.mockRejectedValue(new Error('AccessError: you cannot read this record'))
    const store = createTestStore([instance()], 'inst-1')

    await expect(store.getState().listOdooTickets('assigned', 50)).rejects.toThrow('AccessError')
